                        ...
                        <div class="galleries-product galleries-product-01 position-relative d-flex ">
                                ...
                                <div class="box px-0" v-for="file in product.media">
                                    <div class="card p-0 rounded-0 border-0">
                                        <a :href="file.path" class="card-img" data-gtf-mfp="true" target="_blank"
                                           data-gallery-id="02" >
                                            <img v-if="file.type === 'image'"  :src="file.path" alt="product gallery" class="w-100"/>
                                            <video v-else class="w-100 h-100" controls :poster="file.path"><source :src="file.path+'#t=2'">
                                            </video>
                                        </a>
                                    </div>
                                </div>
                            ...
                            <div class="slick-slider slider-nav mx-n1 mx-xl-0"
                                ...
                                <div class="box px-1 px-xl-0 py-4 pt-xl-0" v-for="file in product.media">
                                    <img v-if="file.type === 'image'"  :src="file.path" alt="product gallery" class="w-100"/>
                                    <video v-else class="w-100" ><source :src="file.path+'#t=5'">
                                    </video>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-md-6">
                        <p class="d-flex align-items-center mb-3">
                            <span class="text-line-through"
                                  v-if="product.label === 'sale'">{{ $h.formatPrice(product.price) }}</span>
                            ...
                            <div class="row align-items-end no-gutters mx-n2">
                                <div class="col-sm-7 form-group pl-2 mb-6">
                                    <label class="text-secondary font-weight-600 mb-3"
                                           for="number">{{ $t('quantity') }}: </label>
                                    <div class="input-group position-relative w-100">
                                        <a href="javascript:;" @click="form.quantity--" class="position-absolute pos-fixed-left-center pl-4 z-index-2"><i
                                            class="far fa-minus"></i></a>
                                        <input v-model="form.quantity" :max="product.quantity" name="number" type="number" id="number"
                                ...
                                <div class="col-sm-5 mb-6 w-100 px-2" data-toggle="tooltip" data-placement="left"
                                     title="Add to cart">
                                    <user-loading-button  custom-class="btn btn-lg fs-18 btn-secondary btn-block h-60 border-0">
                                        {{ $t('add-to-cart') }}
                                    </user-loading-button>
                                </div>
                                <div class="mb-6 px-2 col-12" data-toggle="tooltip" data-placement="left"
                                     title="Buy Now">
                                    <user-loading-button  custom-class="px-7 btn-primary btn-lg btn-block h-60 fs-18" @click="checkout()" :loading="loading" type="button">
                                        {{ $t('buy-now') }}
                                    </user-loading-button>
                                </div>
                            </div>
                            ...
                            <li class="row mb-2">
                                ...
                                <ul class="list-inline d-flex align-items-center mb-0 col-8 col-lg-10">
                                    <li class="list-inline-item mr-4">
                                        <share-network :title="product.title" :description="product.description" network="twitter" :url="route('products.show', product.slug)" class="fs-14 lh-1 text-body hover-secondary">
                                            <i class="fab fa-twitter"></i>
                                        </share-network>
                                    </li>
                                    <li class="list-inline-item mr-4">
                                        <share-network :title="product.title"
                                    ...
                                    <li class="list-inline-item">
                                        <share-network :title="product.title" :description="product.description" network="youtube" :url="route('products.show', product.slug)" class="fs-14 lh-1 text-body hover-secondary">
        ...
        <section class="pt-10 pt-lg-12 pb-10 pb-md-6 pb-lg-11 border-top">
            <div class="container">
                <div class="collapse-tabs">
                    <ul class="nav nav-pills mb-5 justify-content-center d-md-flex d-none" id="pills-tab"
                        role="tablist">
                        <li class="nav-item">
                    ...
                    <div class="tab-content bg-white-md shadow-none py-md-5 p-0">
                        <div id="collapse-tabs-accordion-01">
                            <div class="tab-pane tab-pane-parent  show active" id="pills-detail"
                                 role="tabpanel">
                ...
                <div class="row align-items-center justify-content-center no-gutters mx-n2">
                    <div class="form-group pl-2 mb-4 col-4 col-sm-1">
                       <div class="input-group position-relative">
                            <a href="javascript:;" @click="form.quantity--" class="position-absolute pos-fixed-left-center pl-4 z-index-2"><i
                                class="far fa-minus"></i></a>
                            <input v-model="form.quantity" :max="product.quantity" name="number" type="number-quantity" id="number-quantity"
                    ...
                    <div class="mb-4 px-2 col-8 col-sm-4" data-toggle="tooltip" data-placement="left"
                         title="Buy Now">
                        <user-loading-button  custom-class="px-7 btn-primary btn-lg btn-block h-60 fs-18" @click="checkout()" :loading="loading" type="button">
                            {{ $t('buy-now') }}
                        </user-loading-button>
