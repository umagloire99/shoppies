                ...
                <div class="row">
                    <div class="col-6">
                        <div class="form-group">
                            ...
                            <select name="status" id="status" class="form-control">
                                <option
                                    value="1" {{ old('status', $supervisor->status) == "Active" ? 'selected' : null }}>
                                    Active
                                </option>
                                <option
                                    value="0" {{ old('status', $supervisor->status) == "Inactive" ? 'selected' : null }}>
                                    Inactive
                                </option>
                            </select>
                    ...
                    <div class="col-6">
                        ...
                        <select name="receive_email" id="receive-email" class="form-control">
                            <option value="">---</option>
                            <option
                                value="1" {{ old('receive_email', $supervisor->receive_email) == 1 ? 'selected' : null }}>
                                Yes
                            </option>
                            <option
                                value="0" {{ old('receive_email', $supervisor->receive_email) == 0 ? 'selected' : null }}>
                                No
                            </option>
                        </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="roles[]" id="roles" class="form-control select2" multiple="multiple">
                                @forelse($roles as $role)
                                    <option
                                        value="{{ $role->id }}"
                                        {{ in_array($role->name, $supervisorroles) ? 'selected' : null }}>
                                        {{ $role->name }}
                                    </option>
                                @empty
                                @endforelse
                ...
                <div class="row">
                    <div class="col-12">
                        @if($supervisor->user_image)
                            <img class="mb-2" src="{{ asset('storage/images/users/' . $supervisor->user_image) }}"
                                 alt="{{ $supervisor->full_name }}" width="60" height="60">
                            <a class="btn btn-sm btn-danger mb-2"
                               href="{{ route('admin.supervisors.remove_image', $supervisor->id) }}">Remove</a>
                        @else
                            <img src="{{ asset('img/avatar.png') }}" alt="{{ $supervisor->full_name }}" width="60"
                                 height="60">
                        @endif
                        <br>
